...
function parseConfigtxYaml(path) {
	...
	const filename = path + '/artifacts/configtx.yaml';
	if(!fs.existsSync(filename)) {
		throw new Error('File ' + filename + ' not found');
	}
	...
	let configTxObj = {};
	try {
		configTxObj = yaml.load(configTxYaml);
	} catch(err) {
		throw err;
	}
	...
	configTxObj.Organizations.forEach(org => {
		if(!org.AnchorPeers) {
			...
			orgJSON.pem = getBase64TLSCert(path + '/artifacts/' + org.MSPDir + '/tlscacerts/tlsca.' + domain + '-cert.pem');
			...
			const orgPem = getBase64TLSCert(path +  '/artifacts/' + org.MSPDir + '/tlscacerts/tlsca.' + domain + '-cert.pem');
